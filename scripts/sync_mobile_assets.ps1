...
if (-not $Keras) {
    $Jalon5 = Join-Path $SrcModels "jalon5_weak_classes_best.keras"
    ...
    if (Test-Path $Jalon5) { $Keras = $Jalon5 }
    elseif (Test-Path $Jalon4) { $Keras = $Jalon4 }
    elseif (Test-Path $Plantwild) { $Keras = $Plantwild }
    elseif (Test-Path $Baseline) { $Keras = $Baseline }
    else {
        Write-Error "Missing Keras model. Train jalon5 (jalon5_weak_classes_best.keras), jalon4, jalon3, or baseline."
        exit 1
    }
}
